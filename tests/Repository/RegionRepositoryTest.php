...
class RegionRepositoryTest extends TestCase
{
    ...
    {
        $repository = $this->createRepository(FooRegion::class);

        static::assertSame(
            'SELECT r FROM Siganushka\RegionBundle\Tests\Fixtures\FooRegion r WHERE r.parent IS NULL ORDER BY r.parent ASC, r.code ASC',
            $repository->createQueryBuilderByDto('r', new RegionQueryDto(null, null))->getDQL()
        );

        static::assertSame(
            'SELECT r FROM Siganushka\RegionBundle\Tests\Fixtures\FooRegion r WHERE r.parent = :parent ORDER BY r.parent ASC, r.code ASC',
            $repository->createQueryBuilderByDto('r', new RegionQueryDto('100000', null))->getDQL()
        );

        static::assertSame(
            'SELECT r FROM Siganushka\RegionBundle\Tests\Fixtures\FooRegion r WHERE r.parent = :parent AND r.name LIKE :name ORDER BY r.parent ASC, r.code ASC',
            $repository->createQueryBuilderByDto('r', new RegionQueryDto('100000', 'foo'))->getDQL()
        );

        static::assertSame(
            'SELECT r FROM Siganushka\RegionBundle\Tests\Fixtures\FooRegion r WHERE r.name LIKE :name ORDER BY r.parent ASC, r.code ASC',
            $repository->createQueryBuilderByDto('r', new RegionQueryDto(null, 'foo'))->getDQL()
        );
    }

    /**
     * @param class-string<FooRegion> $entityClass
     *
     * @return RegionRepository<FooRegion>
     */
